...
date: 2023-04-21 21:05:21
tags: computer cetwork
categories: 学习
description: 域名系统(DNS)将主机名转换为IP地址，在互联网基础架构中发挥关键作用。在本实验中，我们将仔细查看DNS在客户端的细节。回想一下，客户端在DNS中的角色相对简单——客户端向其本地DNS服务器发送*请求*，并接收一个*响应*。如书中的图2.21和2.22所示，由于DNS分层服务器之间相互通信，可以递归地或迭代地解析客户端的DNS查询请求，而大多数操作是不可见的。然而，从DNS客户端的角度来看，协议非常简 ——将查询指向为本地DNS服务器，并从该服务器接收到响应
hidden: true
---

域名系统(DNS)将主机名转换为IP地址，在互联网基础架构中发挥关键作用。在本实验中，我们将仔细查看DNS在客户端的细节。
<!-- more -->
...
1. 运行*nslookup*以获取一个亚洲的Web服务器的IP地址。该服务器的IP地址是什么？
   ...
   ![image-20230421210837297](Wireshark-Lab-DNS/image-20230421210837297.png)

   查询了哈尔滨理工大学的ip地址, 通过得到的信息可以知道, 该服务器的IP地址为202.118.201.1

   可以发现这是一个来自DNS服务器182.254.116.116的应答, 搜索发现, 这是腾讯的备用域名服务器. 

   ![image-20230421210846142](Wireshark-Lab-DNS/image-20230421210846142.png)
   ...
   如果我们之间对这个DNS服务器发起DNS查询:
    ...
    ![image-20230421210900650](Wireshark-Lab-DNS/image-20230421210900650.png)
...
![image-20230421210910755](Wireshark-Lab-DNS/image-20230421210910755.png)

可以发现权威名字服务器为: ns1.ethz.ch

1. 运行*nslookup*，使用问题2中一个已获得的DNS服务器，来查询Yahoo!邮箱的邮件服务器。它的IP地址是什么？

![image-20230421210921960](Wireshark-Lab-DNS/image-20230421210921960.png)
...
![image-20230421211212041](Wireshark-Lab-DNS/image-20230421211212041.png)

![image-20230421210930573](Wireshark-Lab-DNS/image-20230421210930573.png)

使用腾讯的公共DNS服务器我们可以得到Yahoo的邮件服务器地址.

同时为发现查询`www.mail.yahoo.com`和mail.yahoo.com的结果不同

![image-20230421211245869](Wireshark-Lab-DNS/image-20230421211245869.png)

查询chatGPT得到以下回复: 

![image-20230421211255078](Wireshark-Lab-DNS/image-20230421211255078.png)
...
4. 找到DNS查询和响应消息。它们是否通过UDP或TCP发送？
   ...
   ![image-20230421211313407](Wireshark-Lab-DNS/image-20230421211313407.png)

   通过UDP发送

5. DNS查询消息的目标端口是什么？ DNS响应消息的源端口是什么？

   ![image-20230421211321920](Wireshark-Lab-DNS/image-20230421211321920.png)
...
4. DNS查询消息发送到哪个IP地址？使用ipconfig来确定本地DNS服务器的IP地址。这两个IP地址是否相同？
   ...
   ![image-20230421211341608](Wireshark-Lab-DNS/image-20230421211341608.png)

   

5. 检查DNS查询消息。DNS查询是什么"Type"的？查询消息是否包含任何"answers"？

   ![image-20230421211351099](Wireshark-Lab-DNS/image-20230421211351099.png)
...
8. 检查DNS响应消息。提供了多少个"answers"？这些答案具体包含什么？
   ...
   ![image-20230421211359997](Wireshark-Lab-DNS/image-20230421211359997.png)

   三个answer

   第一个answer: TYPE CNAME, 主机别名到规范主机名的对应关系

   ![image-20230421211409785](Wireshark-Lab-DNS/image-20230421211409785.png)

   包含NAME(域名),TYPE, Class, TTL, Data. CNAME(规范主机名)

   第二个answer: TYPE A, 域名和IP地址

   ![image-20230421211422241](Wireshark-Lab-DNS/image-20230421211422241.png)

   得到了规范主机名的IP地址

   第三个answer: 

   ![image-20230421211431684](Wireshark-Lab-DNS/image-20230421211431684.png)
...
7. 考虑从您主机发送的后续TCP SYN数据包。 SYN数据包的目的IP地址是否与DNS响应消息中提供的任何IP地址相对应？
   ...
   ![image-20230421211445058](Wireshark-Lab-DNS/image-20230421211445058.png)

   可以发现我的主机向这个IP地址发起了TCP的链接请求

8. 这个网页包含一些图片。在获取每个图片前，您的主机是否都发出了新的DNS查询？

   ![image-20230421211456062](Wireshark-Lab-DNS/image-20230421211456062.png)

   有一个对r3.o.lencr.org的DNS查询, 搜索发现这是一个恶意的软件发起的, 目的是给浏览器带来广告(服了....)

   ![image-20230421211504325](Wireshark-Lab-DNS/image-20230421211504325.png)
...
11. DNS查询消息的目标端口是什么？ DNS响应消息的源端口是什么？
    ...
    ![image-20230421211516046](Wireshark-Lab-DNS/image-20230421211516046.png)

    响应: 

    ![image-20230421211522148](Wireshark-Lab-DNS/image-20230421211522148.png)

12. DNS查询消息的目标IP地址是什么？这是你的默认本地DNS服务器的IP地址吗？

    是的

    ![image-20230421211532903](Wireshark-Lab-DNS/image-20230421211532903.png)

13. 检查DNS查询消息。DNS查询是什么"Type"的？查询消息是否包含任何"answers"？

14. TYPE A, 不包含

    ![image-20230421211545749](Wireshark-Lab-DNS/image-20230421211545749.png)

15. 检查DNS响应消息。提供了多少个"answers"？这些答案包含什么？

    三个

    ![image-20230421211557865](Wireshark-Lab-DNS/image-20230421211557865.png)
...
16. DNS查询消息发送到的IP地址是什么？这是您的默认本地DNS服务器的IP地址吗？
    ...
    ![image-20230421211610171](Wireshark-Lab-DNS/image-20230421211610171.png)

17. 检查DNS查询消息。DNS查询是什么"Type"的？查询消息是否包含任何"answers"？

    ![image-20230421211621477](Wireshark-Lab-DNS/image-20230421211621477.png)
...
20. DNS查询消息发送到的IP地址是什么？这是您的默认本地DNS服务器的IP地址吗？如果不是，这个IP地址是什么？
    ...
    ![image-20230421211639835](Wireshark-Lab-DNS/image-20230421211639835.png)
...
21. 检查DNS查询消息。DNS查询是什么"Type"的？查询消息是否包含任何"answers"？
    ...
    ![image-20230421211649155](Wireshark-Lab-DNS/image-20230421211649155.png)

    TYPE: PTR 通过IP反向查询域名

    第二个查询: TYPE A, 查询`www.aiit.or.kr.poly.edu`的IP地址, 目标IP地址是18.72.0.3

    ![image-20230421211656347](Wireshark-Lab-DNS/image-20230421211656347.png)

    第三个查询: TYPE A, 查询`www.aiit.or.kr`的IP地址, 目标IP和上一个查询相同.

    ![image-20230421211702635](Wireshark-Lab-DNS/image-20230421211702635.png)

22. 检查DNS响应消息。提供了多少个"answers"？这些答案包含什么？

    第一个响应: 提供了一个answer

    ![image-20230421211713117](Wireshark-Lab-DNS/image-20230421211713117.png)
    ...
    ![image-20230421211730365](Wireshark-Lab-DNS/image-20230421211730365.png)

    TYPE: SOA(Start Of a zone of Authority)

    ![image-20230421211748643](Wireshark-Lab-DNS/image-20230421211748643.png)

    响应结果为找不到:

    第三个响应:

    ![image-20230421211757311](Wireshark-Lab-DNS/image-20230421211757311.png)
