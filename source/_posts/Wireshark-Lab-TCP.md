---
title: 'Wireshark Lab: TCP'
date: 2023-04-21 20:48:28
tags: computer network
categories: 学习 
description: 在本实验中，我们将详细研究著名的 TCP 协议的行为。 我们将通过从您的电脑向 远程服务器传输一份 150KB 的文件，并分析 TCP 传输内容的发送和接收过程来实现。 我们将研究 TCP 对序列和 确认号的使用，以提供可靠的数据传输;我们将看到 TCP 的拥塞控制算法 – 慢启 动和拥塞避免 – 在过程中，我们将看看 TCP 的接收器发送流量控制的机制。 我们还将简要地观察 TCP 连线的设置，我们还会研究计算机和服务器之间 TCP 连线的性能。
hidden: true
---
...
![image-20230421205628132](Wireshark-Lab-TCP/image-20230421205628132.png)
...
![image-20230421205643562](Wireshark-Lab-TCP/image-20230421205643562.png)

Client IP Addressee: 10.51.172.226 Client Port: 61084

-  用于在客户端计算机和 gaia.cs.umass.edu 之间启动 TCP 连接的 TCP SYN 段的序列号是什么? 标识为 SYN 段的段有什么功能?

![image-20230421205702461](Wireshark-Lab-TCP/image-20230421205702461.png)

如图, SYN序列号为`4095602682`, 标识为SYN的segment用于建立TCP链接

- gaia.cs.umass.edu 发送给客户端计算机以回复 SYN 的 SYNACK 段的序列号是多少? 

`2017202044`

![image-20230421205716096](Wireshark-Lab-TCP/image-20230421205716096.png)

SYNACK 段中的 Acknowledgment 栏位的值是多少? 

为`4095602683`

![image-20230421205729437](Wireshark-Lab-TCP/image-20230421205729437.png)
...
![image-20230421205757558](Wireshark-Lab-TCP/image-20230421205757558.png)

**用于建立链接, 是三次握手的第二次, 表示同样连接请求**

- 包含 HTTP POST 命令的 TCP 区段的序列号是多少? 

![image-20230421205811084](Wireshark-Lab-TCP/image-20230421205811084.png)
...
![image-20230421205823688](Wireshark-Lab-TCP/image-20230421205823688.png)

### 第一段Frame1: 

(发送第一个frame,在 Wireshark中序号为frame69,这是Wireshark捕获到的序号, 由于这是发送数据的第一帧记frame1)

![image-20230421205838620](Wireshark-Lab-TCP/image-20230421205838620.png)
...
![image-20230421205848826](Wireshark-Lab-TCP/image-20230421205848826.png)

ACK(对第一个frame)的ACK

![image-20230421205858659](Wireshark-Lab-TCP/image-20230421205858659.png)

发现这里的ACK number = 4095603435, 而seq1 的Next sequence = 4095603298, 而恰好, frame2的Next sequence = 4095603435

![image-20230415112704099](Wireshark-Lab-TCP/image-20230421205911880.png)
...
![image-20230421205924206](Wireshark-Lab-TCP/image-20230421205924206.png)

对比ACK1的Timestamps: 

![image-20230421205931912](Wireshark-Lab-TCP/image-20230421205931912.png)
...
![image-20230421205941895](Wireshark-Lab-TCP/image-20230421205941895.png)

### **第二段:**

 我们直接去找, 收到的第二个ACK报文

![image-20230421210002459](Wireshark-Lab-TCP/image-20230421210002459.png)
...
![image-20230421210012633](Wireshark-Lab-TCP/image-20230421210012633.png)

根据wireshark的分析, 可以知道, RTT为0.409390000s

![image-20230421210034300](Wireshark-Lab-TCP/image-20230421210034300.png)

### 第三段: 

![image-20230415135344532](Wireshark-Lab-TCP/image-20230421210044526.png)

### 第四段: 

![image-20230421210050923](Wireshark-Lab-TCP/image-20230421210050923.png)

### 第五段: 

![image-20230421210058078](Wireshark-Lab-TCP/image-20230421210058078.png)



### 第六段

![image-20230421210120044](Wireshark-Lab-TCP/image-20230421210120044.png)
...
![image-20230421210135957](Wireshark-Lab-TCP/image-20230421210135957.png)

- What is the length of each of the first six TCP segments?4 前六个 TCP 区段的长度是多少?

  ![image-20230421210144892](Wireshark-Lab-TCP/image-20230421210144892.png)

	![image-20230421210152513](Wireshark-Lab-TCP/image-20230421210152513.png)
...
- What is the minimum amount of available buffer space advertised at the received for the entire trace? Does the lack of receiver buffer space ever throttle the sender? 对于整个跟踪包，收到的最小可用缓冲区空间量是多少? 缺少接收缓冲区空间是否会限制发送方传送 TCP 区段?
  ...
  ![image-20230421210204402](Wireshark-Lab-TCP/image-20230421210204402.png)
...
- Are there any retransmitted segments in the trace file? What did you check for (in the trace) in order to answer this question?
  ...
  ![image-20230421210221963](Wireshark-Lab-TCP/image-20230421210221963.png)

  通常到收到了不同的两个冗余ACK, 但这不一定意味着丢包, 这是因为分组乱序到达接受方导致的.

  ![image-20230421210234982](Wireshark-Lab-TCP/image-20230421210234982.png)
...
- How much data does the receiver typically acknowledge in an ACK? Can you identify cases where the receiver is ACKing every other received segment (see Table 3.2 on page 250 in the text).
  ...
  按照表3-2的说明, 接受方最多确认两个段, 但是观察了捕获结果, 发现很多事确认了超过两个段段情况, 比如在下图中, 收到的第一个ACK确认了69,70两段. 这是一次典型的delay ACK

  ![image-20230421210252350](Wireshark-Lab-TCP/image-20230421210252350.png)
  ...
  让我们问问万能的chatGPT吧: 
   ...
   ![image-20230421210304585](Wireshark-Lab-TCP/image-20230421210304585.png)
  
   **最多可以合并2~5个报文段哦!!! chatGPT yyds!**
  
   ![image-20230421210315184](Wireshark-Lab-TCP/image-20230421210315184.png)
...
- What is the throughput (bytes transferred per unit time) for the TCP connection? Explain how you calculated this value.TCP 连接的吞吐量(每单位时间传输的节数)是多少? 解释你如何计算 这个值。
  ...
  ![image-20230421210352640](Wireshark-Lab-TCP/image-20230421210352640.png)

  ![image-20230421210406967](Wireshark-Lab-TCP/image-20230421210406967.png)

  总传输时间为5.455830-0.026477=2.50933秒。

  因此，tcp连接的吞吐量计算为152936/2.50933=60946.945997537191202B/s = 60KB/s。

  ![image-20230421210418067](Wireshark-Lab-TCP/image-20230421210418067.png)
...
- 使用时序图(Stevens)绘图工具查看从客户端发送到 gaia.cs.umass.edu 服务 器的区段的序列号与时间关系图。您能否确定 TCP 的慢启动阶段的开始和 结束位置，以及拥塞避免接管的位置? 评论测量数据与我们在文本中研究 的 TCP 的理想化行为的不同之处。
  ...
  ![image-20230421210435578](Wireshark-Lab-TCP/image-20230421210435578.png)
